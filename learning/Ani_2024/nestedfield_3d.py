...
This script simulates the trajectories of particles released every 3 minutes from 8:00pm to 11:00pm in 3D within a reef cluster domain nested within the GBR1km grid.
Particle trajectories are simulated for 30 days and are output every 15 minutes.  

This code was based on example codes provided by OceanParcels on NEMO 3D and NestedFields 

To run the script use the Syntax: python nestedfield_3d.py [Cluster] [Year of release] [Day of release] [Cluster spatial data]

@author: Chinenye Ani, AIMS 
"""

from parcels import Field, NestedField, FieldSet, ParticleSet, JITParticle, Variable, ParticleFile, ErrorCode, AdvectionRK4_3D
import numpy as np
import datetime
...
dimensions = {'U': {'lon': 'longitude', 'lat': 'latitude', 'depth': 'zc', 'time': 'time'},
              ...
              'W': {'lon': 'longitude', 'lat': 'latitude', 'depth': 'zc', 'time': 'time'}}
...
dimensionsW={'lon':'longitude', 'lat':'latitude', 'depth': 'zc', 'time':'time'}
...
W_recom=Field.from_netcdf(data, ('W_recom','w'), dimensionsW, fieldtype='W', chunksize = 'auto', allow_time_extrapolation = True)

#add RECOM fieldset to GBR fieldset
fieldset.add_field(U_recom)
fieldset.add_field(V_recom)
fieldset.add_field(W_recom)

#create nested field
U=NestedField('U', [fieldset.U_recom, fieldset.U])
V=NestedField('V', [fieldset.V_recom, fieldset.V])
W=NestedField('W', [fieldset.W_recom, fieldset.W])
nest=FieldSet(U, V)
nest.add_field(W)
...
# (in squared distance) to a given lat/lon value.
...
output_file = pset.ParticleFile(name= args.cluster + '/outputs/' + args.year + '/' + args.cluster + '_trajectory_' + args.year + '_d' + args.day + '3d.zarr', 
                                outputdt=delta(hours=out_dt))
pset.execute(AdvectionRK4_3D, runtime=delta(days=run_duration), dt=delta(minutes=run_dt), 
