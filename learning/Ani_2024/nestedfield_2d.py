...
This script simulates the trajectories of particles released every 3 minutes from 8:00pm to 11:00pm in 2D within a reef cluster domain nested within the GBR1km grid.
...
To run the script use the Syntax python nestedfield_2d.py [Cluster] [Year of release] [Day of release] [Cluster spatial data]

@author Chinenye Ani, AIMS 
"""

from parcels import Field, NestedField, FieldSet, ParticleSet, JITParticle, Variable, ParticleFile, ErrorCode, AdvectionRK4
import numpy as np
import datetime
...
output_file = pset.ParticleFile(name= args.cluster + 'outputs' + args.year + '' + args.cluster + '_trajectory_' + args.year + '_d' + args.day + '2d.zarr', 
                                outputdt=delta(hours=out_dt))
pset.execute(AdvectionRK4, runtime=delta(days=run_duration), dt=delta(minutes=run_dt), 
